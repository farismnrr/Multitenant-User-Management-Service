...
describe('PATCH /api/users/uploads - Upload Profile Picture', () => {
    ...
    test('Scenario 1: Upload without JWT', async () => {
        try {
            await axios.patch(`${BASE_URL}/api/users/uploads`, {}, {
    ...
    test('Scenario 3: Upload invalid file type', async () => {
        try {
            const form = new FormData();
            form.append('file', Buffer.from('just text'), { filename: 'file.txt', contentType: 'text/plain' });
    ...
    test('Scenario 5: Security: Malicious File Extension (RCE)', async () => {
        try {
            const form = new FormData();
            form.append('file', Buffer.from('<?php echo "hack"; ?>'), { filename: 'exploit.php', contentType: 'application/x-php' });
    ...
    test('Scenario 6: Security: Double Extension / MIME Type Bypass', async () => {
        ...
        try {
            const form = new FormData();
            form.append('file', Buffer.from('fake php content'), { filename: 'exploit.jpg.php', contentType: 'image/jpeg' });
    ...
    test('Scenario 7: Security: Path Traversal in Filename', async () => {
        const form = new FormData();
        form.append('file', Buffer.from('image data'), { filename: '../../etc/passwd', contentType: 'image/jpeg' });
    ...
    test('Scenario 8: Security: Image Tragic / Malformed Image (DoS)', async () => {
        try {
            const form = new FormData();
            form.append('file', Buffer.from('corrupted binary junk'), { filename: 'corrupt.jpg', contentType: 'image/jpeg' });
