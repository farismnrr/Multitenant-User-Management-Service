...
describe('PUT /api/tenants/:id - Update Tenant', () => {
    ...
    let authToken = '';
    let tenantId = '';
    let conflictingTenantName = "Conflicting_Tenant_" + Date.now();
    const testUser = {
    ...
    beforeAll(async () => {
        try {
            ...
            const tRes = await axios.post(`${BASE_URL}/api/tenants`, {
    ...
    test('Scenario 1: Update without JWT', async () => {
        try {
            await axios.put(`${BASE_URL}/api/tenants/${tenantId}`, { name: "New Name" }, {
    ...
    test('Scenario 2: Update non-existent tenant', async () => {
        ...
        try {
            await axios.put(`${BASE_URL}/api/tenants/${fakeId}`, { name: "New Name" }, {
                headers: {
                    'X-API-Key': API_KEY,
    ...
    test('Scenario 3: Update tenant with valid data', async () => {
        ...
        const response = await axios.put(`${BASE_URL}/api/tenants/${tenantId}`, {
            name: newName,
