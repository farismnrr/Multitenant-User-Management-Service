...
impl UserRepositoryTrait for UserRepository {
    ...
    async fn find_by_id(&self, id: Uuid) -> Result<Option<User>, AppError> {
        ...
        let user = UserEntity::find_by_id(id)
            .filter(crate::domains::user::entities::user::Column::DeletedAt.is_null())
            .one(&*self.db)
            .await
    ...
    async fn find_all(&self) -> Result<Vec<User>, AppError> {
        let users = UserEntity::find()
            .filter(crate::domains::user::entities::user::Column::DeletedAt.is_null())
            .order_by_desc(crate::domains::user::entities::user::Column::CreatedAt)
            .all(&*self.db)
            .await
            .map_err(|e| AppError::DatabaseError(e.to_string()))?;
    ...
    async fn delete(&self, id: Uuid) -> Result<(), AppError> {
        ...
        let mut user: crate::domains::user::entities::user::ActiveModel = existing.unwrap().into();
    ...
    async fn find_by_email_with_deleted(&self, email: &str) -> Result<Option<User>, AppError> {
        let user = UserEntity::find()
            .filter(crate::domains::user::entities::user::Column::Email.eq(email))
            .one(&*self.db)
            .await
            .map_err(|e| AppError::DatabaseError(e.to_string()))?;
