...
export function parseError(error) {
  ...
  switch (status) {
    ...
    case 409:
      ...
      if (
        apiMessage?.includes("Invalid credentials for account linking") ||
        apiMessage?.includes("Identity anchor conflict")
      ) {
        return {
          message:
            "This email is already registered. Please enter your existing password in the password fields to link this account.",
          type: ERROR_TYPES.CREDENTIAL,
        };
      }
      if (
        apiMessage?.includes("Email already exists") ||
        apiMessage?.includes("Username already exists")
      ) {
        return {
